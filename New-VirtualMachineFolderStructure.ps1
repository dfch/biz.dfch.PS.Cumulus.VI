...
Creates a new vCenter VM Folderstructure



.DESCRIPTION

Creates a new vCenter VM Folderstructure in the Datacenter of the given Virtual Machine. The targetFolderStructure is String containing the Folder/Subfolders separated by default with '\'
...
.PARAMETER targetFolderStructure

The desired folderStructure


.PARAMETER targetFolderStructureSeparator

The separator used to separate the Folders/Subfolders in the given targetFolderStructure string. Default '\'


.PARAMETER moveVM

If moveVM is selected, the Virtual Machine will be moved to the last folder of the given folderStructure


.EXAMPLE

Create a vCenter folder structure in the parent Datacenter of the VM.
$VM = Get-VM -Name 'VM01'
New-VirtualMachineFolderStructure -vm $VM -targetFolderStructure 'Folder\Folder2\Folder3'


.EXAMPLE

Create a vCenter Folder structure with a custom separator in the parent Datacenter of the VM.
$VM = Get-VM -Name 'VM01'
New-VirtualMachineFolderStructure -vm $VM -targetFolderStructure 'Cumulus/cumulus2/cumulus3/cumulus4/cumulus5' -targetFolderStructureSeparator '/'


.EXAMPLE

Create a vCenter Folder structure and move the given VM to Folder3
$VM = Get-VM -Name 'VM01'
New-VirtualMachineFolderStructure -vm $VM -targetFolderStructure 'Folder\Folder2\Folder3' -moveVM


.LINK

Online Version: http://dfch.biz/biz.dfch.PS/Cumulus/Utilities/New-VirtualMachineFolderStructure
...
(
	...
	[ValidateNotNullOrEmpty()]
	[string] $targetFolderStructure = 'Cumulus\VM\Production'
	,
	[Parameter(Mandatory = $false, Position = 2)]
	[ValidateNotNullOrEmpty()]
	[string] $targetFolderStructureSeparator = '\'
	,
...
try
{
	...
	#Validate VM Object
	Log-Debug -fn $fn -msg ("Validate VM Object")
	if(($null -eq $VM) -or ('VirtualMachineImpl' -ne $VM.getType().name))
	{
		$e = New-CustomErrorRecord -m "Failed to verify the specified VM."
		 throw($gotoError);
	}
	
	Log-Debug -fn $fn -msg ("Validate DC Object")
	$DC = Get-Datacenter -VM $VM -ea SilentlyContinue
	if(($null -eq $DC) -or ('DatacenterImpl' -ne $DC.getType().name))
	{
		$e = New-CustomErrorRecord -m "Failed to determine the DC for the given VM."
		 throw($gotoError);
	}
	Log-Debug -fn $fn -msg ('Validate Parent Folder Object')
	$parentLocation =  get-folder -Type VM -Location $DC -Name VM -ea SilentlyContinue | select -first 1
	if(($null -eq $parentLocation) -or ('FolderImpl' -ne $parentLocation.getType().name))
	{
		$e = New-CustomErrorRecord -m "Failed to determine the parent Folder location."
		 throw($gotoError);
	}

	foreach( $folderName in $targetFolderStructure.Split($targetFolderStructureSeparator))
	{
		if( $folderName )
		{	
			Log-Debug -fn $fn -msg ('Verify if folder already exists: {0} - ParentLocation: {1}' -f $folderName, $parentLocation.Name )
			$curFolder = Get-Folder -Name $folderName -Location $parentLocation -ea SilentlyContinue
			if(($null -eq $curFolder) -or ('FolderImpl' -ne $curFolder.getType().name))
			{
				Log-Debug -fn $fn -msg ('Create Folder: {0}' -f $folderName )
				$curFolder = New-Folder -Name $folderName -Location $parentLocation
				Log-Debug -fn $fn -msg ('Validate created folder Object: {0}' -f $folderName )
				if(($null -eq $curFolder) -or ('FolderImpl' -ne $curFolder.getType().name))
				{
					$e = New-CustomErrorRecord -m "Failed to verify the created Folder."
					throw($gotoError);
				}
			} 
			else 
			{
					Log-Debug -fn $fn -msg ('Folder: {0} already exists' -f $folderName )
			}
			
			Log-Debug -fn $fn -msg ('Set current folder as parent folder: {0}' -f $folderName )
			$parentLocation = $curFolder
		}
	}
	
	if( $moveVM -and $VM )
	{
		Log-Debug -fn $fn -msg ('{0} Move VM to destination folder: {1}' -f $vm.name, $folderName )
		$VM = Move-VM -VM $VM -location $curFolder -ea Stop
	}
...
if($MyInvocation.ScriptName) { Export-ModuleMember -Function New-VirtualMachineFolderStructure; } 


# SIG # Begin signature block
# MIIW3AYJKoZIhvcNAQcCoIIWzTCCFskCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUkxHFNOJG+TZg4dZNwecPOhHL
# 7q2gghGYMIIEFDCCAvygAwIBAgILBAAAAAABL07hUtcwDQYJKoZIhvcNAQEFBQAw
...
# AQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQmfBQZ0mH+fzOGE9wb
# Z0joUv9vKzANBgkqhkiG9w0BAQEFAASCAQCyivNIYMekGhtfis/T5q/Ivf5H/gS5
# lW4fIPmnAP9oAGM64rNIGSiVaEKTHtTNOhmZrPUyGQ6EjgRvNpyDXucsl+7BHL4Z
# qHQFBZJvkmpjjfyBefVqJdUwUuHLnxBPIrkIsZa8NS8x8LFRQmelvVXfOegMWW8h
# XyaLy1lkxPqRWehmm1zIZCVo2rvYs3c+mIHuM4mglcTnne/mV9tTlBB1UwJLvXVN
# xl7WDmdsZjOra0Cl1PbLEFONvk2TndtDl2HJTMyvDNZXTRzEnL2M2XVt81pAdUOn
# 8nm0WaLo8ckl4Dy24NGeNvMRBL7vU09Zx6sdOOEcBjFFiwt4fsiO62fgoYICojCC
# Ap4GCSqGSIb3DQEJBjGCAo8wggKLAgEBMGgwUjELMAkGA1UEBhMCQkUxGTAXBgNV
# BAoTEEdsb2JhbFNpZ24gbnYtc2ExKDAmBgNVBAMTH0dsb2JhbFNpZ24gVGltZXN0
# YW1waW5nIENBIC0gRzICEhEhQFwfDtJYiCvlTYaGuhHqRTAJBgUrDgMCGgUAoIH9
# MBgGCSqGSIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTE0MTIw
# MzA4MjMyMFowIwYJKoZIhvcNAQkEMRYEFPSf4kicf2DSHDPyIuGEABFmtLsIMIGd
# BgsqhkiG9w0BCRACDDGBjTCBijCBhzCBhAQUjOafUBLh0aj7OV4uMeK0K947NDsw
# bDBWpFQwUjELMAkGA1UEBhMCQkUxGTAXBgNVBAoTEEdsb2JhbFNpZ24gbnYtc2Ex
# KDAmBgNVBAMTH0dsb2JhbFNpZ24gVGltZXN0YW1waW5nIENBIC0gRzICEhEhQFwf
# DtJYiCvlTYaGuhHqRTANBgkqhkiG9w0BAQEFAASCAQBzPjTfFx/E8SWERkFJ/7jN
# XUYg1E45jkjtBVk/LdjFzElpA7rrmZkvUwYVmC68Ta68hWxcrvyGiw7wZD5s5CrX
# EkDiSC1IX1fUjPxeJ26fGIuCzSexNYH+sRU/KjtC7eLpltZUiIGEZwkBsxBfy6qU
# XMCXAjJW9h0XbLE3bZD64DVvJYBXTgta5ywuC8mvUurG0zh7mZFGTCiq2uxK8Y4i
# XSVv1HqGDo28syivHpZR5+NvcXR0Efnqis/tGwW/fHZtS/0TRhzAUCWB80BafWip
# 3d0uu103sCsXXhdrzUuCR44H9NiVlPAcRgzj8i//G8AenAM4QUuxqoNJSR5cPRDy
